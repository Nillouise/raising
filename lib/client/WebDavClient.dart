...
class WebDavClient {
  ...
  WebDavClient(String host, String username, String password, String path, {String protocol, int port}) {
    this.baseUrl = host;
    if (!host.startsWith("https://") && !host.startsWith("http://") && protocol != "") {
    ...
    this.httpClient.addCredentials(Uri.parse(this.baseUrl), "", HttpClientBasicCredentials(username, password));
  }
  ...
  ///
  Future<HttpClientResponse> _send(String method, String path, List<int> expectedCodes, {Uint8List data, Map headers}) async {
    return await retry(() => this.__send(method, path, expectedCodes, data: data, headers: headers), retryIf: (e) => e is WebDavException, maxAttempts: 5);
  }

  /// send the request with given [method] and [path]
  Future<HttpClientResponse> __send(String method, String path, List<int> expectedCodes, {Uint8List data, Map headers}) async {
  ...
  /// download [remotePath] and store the response file contents to String
  Future<HttpClientResponse> downloadToBinary(String remotePath) async {
    HttpClientResponse response = await this._send('GET', remotePath, [200]);
    return response;
  }
  ...
  Future<List<FileInfo>> ls({String path, int depth = 1}) async {
    ...
    HttpClientResponse response = await this._send('PROPFIND', path, [207, 301], headers: userHeader);
