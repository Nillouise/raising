...
class FileInfo {
  ...
  String get name {
    if (this.isDirectory) {
      return Uri.decodeFull(this.path.substring(0, this.path.lastIndexOf("/")).split("/").last);
    }
...
List<FileInfo> treeFromWebDavXml(String xmlStr) {
  ...
  findAllElementsFromDocument(xmlDocument, "response").forEach((response) {
    ...
    findElementsFromElement(findElementsFromElement(response, "propstat").first, "prop").forEach((element) {
      ...
      final creationTime = creationTimeElements.isNotEmpty ? creationTimeElements.single.text : DateTime.fromMillisecondsSinceEpoch(0).toIso8601String();

      // Add the just found file to the tree
      tree.add(new FileInfo(davItemName, contentLength, lastModified, DateTime.parse(creationTime), ""));
    });
  });
...
List<xml.XmlElement> findAllElementsFromDocument(xml.XmlDocument document, String tag) => document.findAllElements(tag, namespace: '*').toList();

List<xml.XmlElement> findElementsFromElement(xml.XmlElement element, String tag) => element.findElements(tag, namespace: '*').toList();
