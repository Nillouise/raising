...
class SmbExploreFile implements ExploreFile {
  ...
  Future<ExtractCO> loadFileFromZip(String absPath, int index, {int fileSize}) async {
    try {
      var arguments = {"hostPO": hostPO.toJson(), "absPath": absPath, "index": index};
      final Map<dynamic, dynamic> result = await SmbChannel.methodChannel.invokeMethod("smbExtract", arguments);
      var map = Map<String, dynamic>.from(result);
      map["indexPath"] = (Map<int, String>.from(map["indexPath"]))?.map((k, e) => MapEntry((k).toString(), e as String));
      map["indexContent"] = (Map<int, Uint8List>.from(map["indexContent"]))?.map((k, e) => MapEntry(k as int, e as Uint8List));
  ...
  Future<List<ExploreCO>> queryFiles(String path) async {
    ...
    try {
      List<dynamic> result = await SmbChannel.methodChannel.invokeMethod("smbQueryFiles", arguments);
      return result.map((e) {
        var exploreCO = ExploreCO.fromJson(Map<String, dynamic>.from(e));
        return exploreCO..absPath = p.join(path, exploreCO.filename);
      }).toList();
    } catch (e) {
...
//  WebDavClient client = WebDavClient("http://109.131.14.238:57203/", "", "", "");
  ...
  WebdavExploreFile(this.hostPO) {
    //TODO:还需要完善
    client = WebDavClient(hostPO.hostname, hostPO.username, hostPO.password, "");
  }
...
//    final Map<dynamic, dynamic> result = await SmbChannel.methodChannel.invokeMethod("webdavExtract", {"recallId": path, "fileSize": fileSize, "index": index});
    ...
    var arguments = {"absPathLink": client.baseUrl + absPath, "username": client.username, "password": client.password, "fileSize": curFileSize, "index": index};
    final Map<dynamic, dynamic> result = await SmbChannel.methodChannel.invokeMethod("webdavExtract", arguments);
    var map = Map<String, dynamic>.from(result);
    map["indexPath"] = (Map<int, String>.from(map["indexPath"]))?.map((k, e) => MapEntry((k).toString(), e as String));
    map["indexContent"] = (Map<int, Uint8List>.from(map["indexContent"]))?.map((k, e) => MapEntry(k as int, e as Uint8List));
  ...
  ///TODO:这里可能要做个缓存功能，不能不停刷新问题就大。或者考虑一下翻页后的位置能否保持原样。
  @override
  Future<List<ExploreCO>> queryFiles(String path) async {
